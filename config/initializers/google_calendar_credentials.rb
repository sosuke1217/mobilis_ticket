...
begin
    ...
    if client_id.present? && client_secret.present?
      unless File.exist?(credentials_path)
        ...
        begin
          ...
          Rails.logger.error "❌ Failed to create Google Calendar credentials file: #{e.message}"
  ...
  Rails.logger.error "❌ Error in google_calendar_credentials initializer: #{e.message}"
  Rails.logger.error "❌ Backtrace: #{e.backtrace.first(5).join("\n")}"
